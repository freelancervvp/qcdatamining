...
library(caret)
Sys.time()
gbm.lte.d.7 <- gbm(log10(download_speed) ~ new_month + new_weekday + new_hour + tz
                   + new_network_country + network_id
                   + phone_model + location_precision
                   + rsrp + rsrq + rssnr
                   + icmp_ping_time + icmp_ping_packet_loss + icmp_ping_range 
                   + app_version_code + test_type, 
                   data = na.roughfix(temp.tr.d.train.lte), 
                   ...
                   keep.data = TRUE,
                   verbose = TRUE)
gbm.lte.d.7 <- gbm(log10(download_speed) ~ new_month + new_year + new_weekday + new_hour + tz
...
gbm.lte.d.predict.test <- 10^(predict.gbm(object = gbm.lte.d.7, newdata = temp.tr.d.test.lte, 1000))
